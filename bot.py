...
async def account_details(callback: CallbackQuery):
    """
    Просмотр и управление отдельным аккаунтом.
    Показывает информацию об аккаунте и кнопки для действий.
    """
...
async def run_stories(user_id: int, account: Dict, media_data: BytesIO, is_video: bool, usernames: List[str], delay: int):
    ...
    Публикует истории для всех (видимость - всем) и отмечает указанных пользователей.
    """
    ...
    try:
        ...
        # Получаем информацию о пользователях для отметок
        resolved_users = []
        ...
        for i in range(0, len(resolved_users), 4):
            ...
            try:
                # Создаем медиа в зависимости от типа
                if is_video:
                ...
                # Отмечаем пользователей в истории
                # Преобразуем user_id каждого пользователя в InputUser
                mentioned_ids = []
                for user in batch:
