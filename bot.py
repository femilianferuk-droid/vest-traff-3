...
async def run_stories(user_id: int, account: Dict, media_data: BytesIO, is_video: bool, usernames: List[str], delay: int):
    """
    Фоновый процесс создания историй с отметками людей.
    Публикует истории для всех.
    """
    try:
        client = TelegramClient(
            StringSession(account['session_string']),
        ...
        if not resolved_users:
            await bot.send_message(
                user_id,
                f'<tg-emoji emoji-id="{Emoji.cross}">❌</tg-emoji> '
                f'Не удалось найти ни одного пользователя из списка'
            )
            return
        
        # Создаем истории для каждой группы
        for i in range(0, len(resolved_users), 4):
            if user_id in active_stories and active_stories[user_id]["stop"]:
                break
            
            batch = resolved_users[i:i+4]
            
            try:
                # Создаем media_areas для отметки пользователей
                media_areas = []
                positions = [
                    (0.25, 0.3, 0.35, 0.4),   # левый верхний
                    (0.65, 0.3, 0.75, 0.4),   # правый верхний
                    (0.25, 0.65, 0.35, 0.75), # левый нижний
                    (0.65, 0.65, 0.75, 0.75), # правый нижний
                ]
                
                for idx, user in enumerate(batch):
                    if idx < len(positions):
                        pos = positions[idx]
                        area = MediaAreaSuggestedReaction(
                ...
                await client(SendStoryRequest(
                    peer='me',
                    media=media,
                    privacy_rules=[],  # Пустой список = история для всех
                    random_id=random.randint(100000, 999999),
                    media_areas=media_areas if media_areas else None,
                    caption=f"Отметки: {', '.join(['@' + u.username for u in batch if u.username])}"
                ))
            ...
            if i + 4 < len(resolved_users) and delay > 0:
...
async def run_commentator(user_id: int, account: Dict, channel_ids: List[int], comment_text: str):
    ...
    Отслеживает новые посты и оставляет комментарии в discussion группе.
    """
    try:
        client = TelegramClient(
            StringSession(account['session_string']),
        ...
        # Получаем discussion группы для каналов
        channel_discussions = {}
        for chat_id in channel_ids:
            try:
                full_chat = await client(GetFullChannelRequest(channel=chat_id))
                if full_chat.full_chat.linked_chat_id:
                    channel_discussions[chat_id] = full_chat.full_chat.linked_chat_id
                    logger.info(f"Found discussion group {full_chat.full_chat.linked_chat_id} for channel {chat_id}")
                else:
                    logger.warning(f"No discussion group for channel {chat_id}")
            except Exception as e:
                logger.error(f"Error getting discussion for {chat_id}: {e}")
        
        if not channel_discussions:
            await bot.send_message(
                user_id,
                f'<tg-emoji emoji-id="{Emoji.cross}">❌</tg-emoji> '
                f'Не найдены группы обсуждений для выбранных каналов.\n'
                f'Убедитесь, что в каналах включены комментарии.'
            )
            return
        
        # Обработчик новых сообщений в каналах
        @client.on(events.NewMessage(chats=list(channel_discussions.keys())))
        async def handler(event):
            ...
            try:
                ...
                discussion_id = channel_discussions.get(event.chat_id)
                if discussion_id:
                    ...
                    logger.info(f"Comment posted in discussion {discussion_id} for channel {event.chat_id}")
        ...
        await bot.send_message(
            ...
            f'Отслеживается каналов: {len(channel_discussions)}\n'
