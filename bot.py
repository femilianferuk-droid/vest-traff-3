...
class Emoji:
    ...
    command_emoji = "5940433880585605708"
    crypto = "5260752406890711732"
    config = "5870528606328852614"
...
class Database:
    ...
    async def init(self):
        ...
        # Получаем текущий event loop для асинхронных операций
        loop = asyncio.get_running_loop()
        
        # Создаем асинхронные обертки для работы с БД
        ...
        # Сохраняем методы как атрибуты экземпляра
        self._execute = _exec
        self._fetch = _fetch
        ...
        # Таблица токенов CryptoBot
        await self._execute('''
            CREATE TABLE IF NOT EXISTS user_crypto_tokens (
                user_id BIGINT PRIMARY KEY,
                token TEXT NOT NULL
            )
        ''')
    ...
    async def create_user(self, user_id: int):
        ...
        await self._execute('INSERT INTO users (user_id) VALUES (%s) ON CONFLICT DO NOTHING', user_id)
    
    async def has_subscription(self, user_id: int) -> bool:
        """Проверить наличие активной подписки"""
        row = await self._fetchrow("SELECT subscription_until FROM users WHERE user_id = %s", user_id)
    ...
    async def add_subscription(self, user_id: int, days: int):
        ...
        current = await self._fetchrow("SELECT subscription_until FROM users WHERE user_id = %s", user_id)
        ...
        await self._execute('UPDATE users SET subscription_until = %s WHERE user_id = %s', new_until, user_id)
    ...
    async def get_user_accounts(self, user_id: int) -> List[Dict]:
        ...
        return [{"id": r['id'], "phone": r['phone'], "is_active": r['is_active']} for r in rows]
    
    async def get_account_count(self, user_id: int) -> int:
        """Получить количество аккаунтов пользователя"""
        count = await self._fetchval("SELECT COUNT(*) FROM accounts WHERE user_id = %s", user_id)
    ...
    async def get_promo(self, code: str) -> Optional[Dict]:
        ...
        row = await self._fetchrow("SELECT * FROM promocodes WHERE code = %s", code.upper())
        return dict(row) if row else None
    
    async def activate_promo(self, user_id: int, promo_id: int):
        """Активировать промокод"""
        await self._execute('INSERT INTO promo_activations (user_id, promo_id) VALUES (%s, %s)', user_id, promo_id)
        await self._execute('UPDATE promocodes SET current_activations = current_activations + 1 WHERE id = %s', promo_id)
    ...
    async def get_instruction(self) -> str:
        ...
        row = await self._fetchrow("SELECT text FROM instructions ORDER BY id DESC LIMIT 1")
        if row and row['text']:
            return row['text']
    ...
    async def set_crypto_token(self, user_id: int, token: str):
        """Сохранить токен CryptoBot"""
        await self._execute(
            'INSERT INTO user_crypto_tokens (user_id, token) VALUES (%s, %s) '
            'ON CONFLICT (user_id) DO UPDATE SET token = %s',
            user_id, token, token
        )
    
    async def get_crypto_token(self, user_id: int) -> Optional[str]:
        """Получить токен CryptoBot"""
        row = await self._fetchrow('SELECT token FROM user_crypto_tokens WHERE user_id = %s', user_id)
...
# Временное хранилище для авторизации аккаунтов
auth_sessions = {}
...
def get_main_menu():
    """
    Главное меню бота.
    Содержит кнопки для навигации по основным разделам.
    """
...
def get_functions_menu():
    """
    Меню функций бота.
    Содержит кнопки для доступа к различным функциям.
    """
...
def get_accounts_menu():
    ...
    Позволяет добавлять и просматривать аккаунты.
    """
    kb = InlineKeyboardBuilder()
...
def get_accounts_list(accounts: List[Dict]):
    ...
    Отображает телефон и статус каждого аккаунта.
    """
    kb = InlineKeyboardBuilder()
    for acc in accounts:
        status_emoji = Emoji.check if acc['is_active'] else Emoji.cross
        kb.button(text=acc['phone'], callback_data=f"acc_{acc['id']}", icon_custom_emoji_id=status_emoji)
...
def get_profile_menu():
    ...
    Содержит информацию о подписке и управление ею.
    """
    kb = InlineKeyboardBuilder()
...
def get_subscription_keyboard():
    ...
    Отображает доступные тарифы с ценами.
    """
    kb = InlineKeyboardBuilder()
    for key, sub in SUBSCRIPTIONS.items():
        kb.button(text=f"{sub['title']} - {sub['price']}₽", callback_data=f"sub_{key}")
...
def get_payment_method_keyboard(sub_key: str):
    ...
    USDT, TON - через CryptoBot, Рубли - через YooMoney.
    """
    kb = InlineKeyboardBuilder()
...
def get_promo_type_keyboard():
    ...
    Скидка в процентах или дни подписки.
    """
    kb = InlineKeyboardBuilder()
...
async def cmd_start(message: Message):
    ...
    welcome_text = (
        ...
        f'<b>Функции</b> - создание историй\n'
...
async def accounts_menu_handler(message: Message):
    ...
    Проверяет подписку и показывает меню управления аккаунтами.
    """
    # Проверка подписки
...
async def functions_menu_handler(message: Message):
    ...
    Показывает меню доступных функций бота.
    """
    if message.from_user.id != ADMIN_ID and not await db.has_subscription(message.from_user.id):
        await message.answer(
            f'<tg-emoji emoji-id="{Emoji.lock_closed}">🔒</tg-emoji> '
...
async def profile_handler(message: Message):
    ...
    Показывает информацию о профиле пользователя и статус подписки.
    """
...
async def process_payment(callback: CallbackQuery):
    """
    Обработка платежа.
    Поддерживает USDT, TON через CryptoBot и Рубли через YooMoney.
    """
...
async def profile_callback(callback: CallbackQuery):
    """
    Просмотр профиля через callback.
    """
...
async def activate_promo_start(callback: CallbackQuery, state: FSMContext):
    ...
    Запрашивает у пользователя код промокода.
    """
    await callback.message.edit_text(
...
async def activate_promo_execute(message: Message, state: FSMContext):
    ...
    Проверяет валидность, количество активаций и активирует промокод.
    """
...
async def process_code(message: Message, state: FSMContext):
    ...
    Завершает авторизацию аккаунта.
    """
...
async def story_menu_handler(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        f'Бот будет создавать истории и отмечать в них людей.',
        reply_markup=kb.as_markup()
    )
...
async def story_receive_photo(message: Message, state: FSMContext):
    ...
    try:
        ...
        # Конвертируем в JPEG для совместимости с Telegram Stories
...
async def story_receive_video(message: Message, state: FSMContext):
    ...
    try:
        ...
        # Конвертируем в BytesIO для удобства
        output = BytesIO()
...
async def story_start(message: Message, state: FSMContext):
    ...
    try:
        ...
        # Запускаем создание историй в фоне
        asyncio.create_task(
            run_stories(
...
async def run_stories(user_id: int, account: Dict, media_data: BytesIO, is_video: bool, usernames: List[str], delay: int):
    """
    Запуск процесса создания историй.
    Первая история создается сразу же, затем с указанной задержкой.
    """
    try:
        # Создаем и подключаем клиент Telethon
        ...
        media_data.seek(0)
        uploaded_file = await client.upload_file(media_data)
        ...
        for i in range(0, len(usernames), 4):
            ...
            # Берем следующую группу из 4 человек
            batch = usernames[i:i+4]
            
            try:
                # Создаем медиа для истории в зависимости от типа
                if is_video:
                    # Для видео используем InputMediaUploadedDocument
                    media = InputMediaUploadedDocument(
                        ...
                        attributes=[
                            DocumentAttributeVideo(
                                ...
                                w=0,  # Ширина определится автоматически
                                h=0,  # Высота определится автоматически
                                round_message=False,
                    ...
                    # Для фото используем InputMediaUploadedPhoto
                ...
                # Отправляем историю через SendStoryRequest
                await client(SendStoryRequest(
                    peer='me',  # Публикуем от своего имени
                    media=media,
                    privacy_rules=[],  # Без ограничений приватности
                    random_id=random.randint(100000, 999999)  # Случайный ID для истории
                ))
            ...
            # Если это не последняя группа и есть задержка - ждем
            if i + 4 < len(usernames) and delay > 0:
                # Ждем указанную задержку с возможностью остановки
        ...
        # Всегда отключаем клиент и очищаем хранилище
        await client.disconnect()
        if user_id in active_stories:
            del active_stories[user_id]
...
async def none_callback(callback: CallbackQuery):
    """
    Заглушка для неиспользуемых callback.
    """
    await callback.answer()
...
async def main():
    ...
    Инициализирует БД и начинает polling.
    """
