...
def get_gift_selection_keyboard(page: int = 0):
    """Клавиатура выбора подарка с пагинацией"""
    kb = InlineKeyboardBuilder()
...
async def run_commentator(user_id: int, account: Dict, channel_ids: List[int], comment_text: str):
    ...
    Отслеживает новые посты и оставляет комментарии напрямую.
    """
    try:
        client = TelegramClient(
            StringSession(account['session_string']),
        ...
        async def handler(event):
            ...
            try:
                ...
                # Пробуем отправить комментарий через comment_to
                try:
                    ...
                    # Если не получилось через comment_to, пробуем просто отправить сообщение
