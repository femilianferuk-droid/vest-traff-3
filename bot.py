...
GIFTS = {
    ...
    "❤️ Сердце 14 февраля": {"id": 5801108895304779062, "stars": 50},
}
...
def get_gift_selection_keyboard(page: int = 0):
    ...
    kb.button(text="🔙 Назад", callback_data="gift_back")
    return kb.as_markup()
...
async def gift_send(message: Message, state: FSMContext):
    ...
    try:
        ...
        # Отправляем запрос через invoke
        from telethon.tl.functions.payments import SendStarGiftRequest as SendStarGift
        
        result = await client.invoke(SendStarGift(
            user_id=receiver,
            gift_id=gift_id,
        ...
        # Если SendStarGiftRequest не найден, пробуем альтернативный способ
        try:
            from telethon import TLRequest
            
            # Создаем запрос вручную
            request = TLRequest(
                method='payments.sendStarGift',
                params={
                    'user_id': receiver,
                    'gift_id': gift_id,
                    'message': caption if caption else "",
